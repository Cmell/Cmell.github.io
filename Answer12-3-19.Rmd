...
Depending on the desired interpretation of $B$, orthogonal contrasts might not be quite right. But given my ignorance of that variable, I will specify a set of orthogonal polynomial contrast codes for it. These would be most useful if $B$ represented something like time. In particualr, the linear and quadratic codes are nice for that because they can be interpreted as the change from beginning to end and the curve of the measurements, respectively.
...
Other orthogonal codes exist that might be more appropriate. For example, if you are really only interested in the difference between, say, levels 1 and 2, then Helmert codes might help more. In this case, the BC1 code would test the tendecy for levels 1 and 2 to differ:
...
For all examples below, I use the first set of polynomial codes. I also assume that B is a time factor and that the effect of interest is the linear trend. This is only because it is easier to talk about with some meaning imbued, but any (correctly specified orthogonal) code or interpretation could work.
...
First, we need a way of taking our observed decisions and modeling them continuously (I'm skipping basically all the math details of logistic regression here because I don't think that is the question). Here's the logit function:

$$\eta_{ij} = log(\frac{P(guilty)_{ij}}{1-P(guilty)_{ij}}$$

**Level 1:**

$$\eta_{ij} = \beta_0 + \beta_{1i} * BC1_{ij} + \beta_{2i} * BC2_{ij} + \beta_{3i} * BC3_{ij} + \beta_{4i} * BC4_{ij}$$

Notice there is no error term in this equation because we are talking about logistic regression.
